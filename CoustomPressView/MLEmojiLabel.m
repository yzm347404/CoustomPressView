...
REGULAREXPRESSION(URLRegularExpression,@"((http[s]{0,1}|ftp)://]{0,1}|[www.]{0,1}[a-zA-Z0-9\\.\\-]+\\.([a-zA-Z]{2,4})(:\\d+)?(/[a-zA-Z0-9\\.\\-~!@#$%^&*+?:_/=<>]*)?)")

//REGULAREXPRESSION(PhoneNumerRegularExpression, @"\\d{3}-\\d{8}|\\d{3}-\\d{7}|\\d{4}-\\d{8}|\\d{4}-\\d{7}|1+[358]+\\d{9}|\\d{8}|\\d{7}")

REGULAREXPRESSION(PhoneNumerRegularExpression, @"^1\\d{10}$")
...
REGULAREXPRESSION(SlashEmojiRegularExpression, @"\\<f0[0-9][0-9]+\\>|\\[[a-zA-Z0-9\\u4e00-\\u9fa5]+\\]")


const CGFloat kLineSpacing = 4.0;
const CGFloat kAscentDescentScale = 0.25; //在这里的话无意义，高度的结局都是和宽度一样

const CGFloat kEmojiWidthRatioWithLineHeight = 1.0;//和字体高度的比例
...
+ (NSDictionary *)emojiDictionary {
    ...
    static dispatch_once_t onceToken;
	dispatch_once(&onceToken, ^{
        NSString *file = [NSString stringWithFormat:@"SKUtils.bundle/face_custom"];
        NSString *plistUrl = [[NSBundle mainBundle] pathForResource:file ofType:@"plist"];
        emojiDictionary = [[NSDictionary alloc] initWithContentsOfFile:plistUrl];
    });
                         ...
                         context:(CGContextRef)c
{
    ...
    for (CFIndex lineIndex = 0; lineIndex < numberOfLines; lineIndex++) {
        ...
        NSMutableDictionary *imagesDictonary = [[BaseHelper sharedBaseHelper] emojiImagesDictionary];
        ...
        for (id glyphRun in (__bridge NSArray *)CTLineGetGlyphRuns(line)) {
            ...
            if (imageName) {
                ...
                UIImage *image = nil;
                NSString *_imageName = [NSString stringWithFormat:@"emoji.pics/%@",imageName];
                if ([imagesDictonary valueForKey:imageName] != nil) {
                    image = [imagesDictonary valueForKey:imageName];
                }else
                {
                    image = [BaseHelper themeImageWithImageName:_imageName];
                    [imagesDictonary setObject:image forKey:imageName];
                }
...
+ (CGSize)sizeForLabelWidth:(CGFloat)width withEmojiText:(NSString *)emojiText textFont:(UIFont *)font lineSpacing:(CGFloat)lineSpacing textInsets:(UIEdgeInsets)textInsets
{
    ...
    emojiLabel.lineSpacing = lineSpacing;
    emojiLabel.textInsets = textInsets;
    
    [emojiLabel setText:emojiText];
    CGFloat _width = [emojiLabel preferredSizeWithMaxWidth:width].width;
    return CGSizeMake(_width, [emojiLabel preferredSizeWithMaxWidth:width].height);
}
...
//    CGFloat _width = [protypeLabel preferredSizeWithMaxWidth:width].width;
//    return CGSizeMake(_width, [protypeLabel preferredSizeWithMaxWidth:width].height);
//}
...
//        protypeLabel = [MLEmojiLabel new];
//        protypeLabel.numberOfLines = 0;
//        protypeLabel.lineSpacing = -3;
//        protypeLabel.font = font;
//        protypeLabel.lineBreakMode = NSLineBreakByWordWrapping;
//        protypeLabel.isNeedAtAndPoundSign = YES;
//        protypeLabel.verticalAlignment = TTTAttributedLabelVerticalAlignmentTop;
//    }
//    [protypeLabel setText:emojiText];
//    
//    return CGSizeMake([protypeLabel preferredSizeWithMaxWidth:width].width, [protypeLabel preferredSizeWithMaxWidth:width].height);
//
//}
